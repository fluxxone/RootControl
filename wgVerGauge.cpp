...
//		return 0;
	...
	uint16_t pix = 0;
	if((yc>=(_height/2 - CHAR_HEIGHT/2) && yc < (_height/2 - CHAR_HEIGHT/2 + CHAR_HEIGHT)) && (xc >= (_width - CHAR_WIDTH*numChars)/2) && (xc < ((_width - CHAR_WIDTH*numChars)/2)+CHAR_WIDTH*numChars))
	{
	...
	if((yval > yc || yc == 1 || yc == _height - 1 || xc == 1 || xc == _width - 1) && yc != 0 && yc != _height && xc != 0 && xc != _width)
		return 0xFFFF ^ pix;
	else
		return 0 ^ pix;
}

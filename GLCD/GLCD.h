...
void LCD_FormatSet(uint8_t);
uint8_t LCD_FormatRead(void); 

void PutCharRL(unsigned short,unsigned short,unsigned char,unsigned short,unsigned short);
...
void drawRect(uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2,uint16_t color,uint8_t seg);
void paintBackground(uint16_t color);
void paintBackgroundGradient(uint16_t color1,uint16_t color2);
void drawMatrix();

void LCD_SetCursor(uint16_t Xpos,uint16_t Ypos);
void LCD_WriteRAM_Prepare();
void LCD_WriteRAM(uint16_t arg);
